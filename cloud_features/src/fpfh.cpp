...
class FPFHFeatures
{
    public:
		...
		std::cout<<"Keypoint one at: "<< keypoint_cloud->at(0).x<<", "<<keypoint_cloud->at(0).y<<", "<<keypoint_cloud->at(0).z<<std::endl;
		std::cout<<"Keypoint one at: "<< keypoint_cloud->at(0).data<<std::endl;
		...
		//std::cout << "Feature 01: " << fpfh_features->points[0] <<std::endl;
		...
		// pcl::visualization::PCLVisualizer viewer("PCL Viewer"); ------------------- only this---
		...
		// std::cout << previous_fpfh_features->size() << std::endl;
		...
		if(previous_fpfh_features->size()> 0 && previous_keypoint_cloud->size()>0){
    		...
    		est.setInputSource(fpfh_features);
    		est.setInputTarget(previous_fpfh_features);
			est.determineReciprocalCorrespondences(*correspondences);
    		// est.determineResiCorrespondences(*correspondences);
		...
		// 	// / refinement transform source using transformation matrix ///////////////////////////////////////////////////////
			...
			// pcl::transformPointCloud(*keypoint_cloud,*transformed_keypoints,transform);
